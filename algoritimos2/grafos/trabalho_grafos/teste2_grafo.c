...
void remove_aresta(grafo *G, int A, int V){
	...
	p=q->prox_aresta;
	aux2=NULL;
	while(p!=NULL && p->arestas!=A){
		aux2=p;
		p=p->prox_aresta;
	}
	if(p!=NULL  && p->arestas==A){
		if(aux2!=NULL){
			aux2->prox_aresta=p->prox_aresta;
			free(p);
		}
		else{
			q->prox_aresta=p->prox_aresta;
			free(p);
		}
	}
}
...
void remove_vertice(grafo *G, int V){
	...
	if(p!=NULL){
		q=p->prox_aresta;
		while(q!=NULL){
			remove_aresta(G,p->vertice,q->arestas);
			q=q->prox_aresta;
		}
		if(G->inicio->vertice==V){
			G->inicio=p->prox;
			free(p);
		}
		else{
			aux=G->inicio;
			while(aux->prox->vertice!=V){
				aux=aux->prox;
			}
			aux->prox=p->prox;
			free(p);
...
void algoritimo_prim(grafo *G, int *d, int *pai, int *key){
	...
	/*while(q!=NULL){
		if(q->prox_aresta!=NULL){
			p=q->prox_aresta;
			while(p!=NULL){
				printf("%d ",weight[q->vertice][p->arestas]);
				p=p->prox_aresta;
			}
			printf("\n");
		}
	...
	while(q!=NULL){
		...
		while(p!=NULL){
			if(pertence(&Q,p->arestas)==1 && weight[q->vertice][p->arestas]<key[p->arestas]){
				//printf("aresta %d || peso %d || key %d\n",p->arestas,weight[q->vertice][p->arestas],key[p->arestas]);
				pai[p->arestas]=q->vertice;
...
//                   A V C
	...
	while(entrada!='@'){
		...
		if(entrada=='V'){
			...
			while(data!=-1){
				if(pertence(&G,data)==1){
					...
					q=G.inicio;
					while(q!=NULL){
						printf("pai[%d] = %d ---> %d\n",q->vertice,pai[q->vertice],distancia[q->vertice]);
						q=q->prox;
					}
				}
				scanf(" %d",&data);
			...
			while(data!=-1){
				...
				while(q!=NULL){
					printf("d[%d]=%d || f[%d]=%d ||pai[%d]=%d\n",q->vertice,distancia[q->vertice],q->vertice,f[q->vertice],q->vertice,pai[q->vertice] );
					q=q->prox;
				}
				scanf("%d",&data);
			}
		...
		//prim
			...
			while(q!=NULL){
				printf("pai[%d]=%d || key[%d]=%d\n",q->vertice,pai[q->vertice],q->vertice,key[q->vertice] );
				q=q->prox;
			}
