...
int buscaminimo(grafo *Q){
	...
	if(q->prox==NULL){
		...
		while(q->prox->prox!=NULL){
			if(q->prox_aresta!=NULL){
				if(q->prox_aresta->custo<=menor){
					menor=q->prox_aresta->custo;
					min=q;
				}
			}
			q=q->prox;
		}
		if(q->prox->prox_aresta!=NULL){
			if(q->prox->prox_aresta->custo<=menor){
				menor=q->prox->vertice;
				q->prox=NULL;
				return menor;
			}
		}
			...
			menor=min->vertice;
			min->vertice=min->prox->vertice;
			min->prox_aresta=min->prox->prox_aresta;
			no *x=min->prox;
			min->prox=x->prox;
			return menor;
		}
