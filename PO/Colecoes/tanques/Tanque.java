...
public class Tanque
{
    ...
    {
        this.x = x; this.y = y; angulo = 90-a; cor = c;
        velocidade = 0;
        estaAtivo = false;
    }
    ...
    {
        if(x + Math.sin(Math.toRadians(angulo))*velocidade<575 &&  x + Math.sin(Math.toRadians(angulo))*velocidade >25){
            x = x + Math.sin(Math.toRadians(angulo))*velocidade;
        }
       
        if(y - Math.cos(Math.toRadians(angulo))*velocidade<375 && y - Math.cos(Math.toRadians(angulo))*velocidade>25){
